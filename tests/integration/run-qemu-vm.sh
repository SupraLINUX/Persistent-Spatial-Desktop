...
if [[ "$modern_hyprland_mode" == "1" ]]; then
    echo "PSD QEMU probe: installing modern-runtime dependencies and representative Ubuntu applications"
    ssh_guest '
        set -e
    ...
    ssh_guest '
        ...
        hyprutils_count="$(awk '"'"'/libhyprutils\\.so\./ {print $1}'"'"' /home/psd/hyprland-modern-ldd.txt | sort -u | wc -l)"
        if [[ "$hyprutils_count" -ne 1 ]]; then
            echo "PSD QEMU probe: mixed hyprutils ABI detected in private runtime" >&2
            grep "libhyprutils.so." /home/psd/hyprland-modern-ldd.txt >&2 || true
            exit 1
        fi
